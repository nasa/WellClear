...
public final class SeparatedInput implements ParameterReader, ErrorReporter {
	...
	public int findHeading(String heading) {
		...
		for (int i = 0; i < header_str.length; i++) {
			if (heading.equals(header_str[i])) {
				rtn = i;
				break;
			}
		}
	...
	public double getColumn(int i) {
      if (i < 0 || i >= line_str.length) {
    	...
    	try {
    		...
    		//for (int j = i-2; j < i+2; j++) f.p(" "+line_str[j]+" "); f.pln(" i = "+i);
   ...
   public void setColumnDelimiters(String delim) {
    ...
    private boolean process_preamble(String str) {
    	...
    	if (fields.length >= 2 && /*(fields[1].equals("=") /*|| fields[1].equals("==")) &&*/ fields[0].length() > 0) {
    	    String id = fields[0].trim();
            if ( ! caseSensitive) id = id.toLowerCase();

             
            StringBuilder value = new StringBuilder(fields[1]);
            for (int i = 2; i < fields.length; i++) {
              value.append("=" + fields[i]);
            } // values become a space-delineated list string

            // String value
            String strv = value.toString().trim();
...
//            	bool = Boolean.TRUE;
//            }
//            Quad<String,Double,String,Boolean> quad = Quad.make(str.substring(str.indexOf(fields[2],str.indexOf("="))).trim(), dbl, unit, bool);
////            Quad<String,Double,String,Boolean> quad = Quad.make(strv, dbl, unit, bool);
