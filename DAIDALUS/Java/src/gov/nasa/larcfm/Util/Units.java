...
public final class Units {
  ...
  static {
    ...
    try {
      ...
      addCoreUnit("hertzAngular", hertzAngular, "rad/s");
    ...
    try {
      addUnit("kg", kg);
      addCoreUnit("slug", slug, "kg");
      addCoreUnit("lbm", lbm, "kg");
    ...
    try {
      ...
      addCoreUnit("ft-lbf", foot * pound_force, "J");
    } catch (UnitException e) {
      throw new Error(e.getMessage());
    }
    ...
    try {
      ...
      addCoreUnit("lbf-ft", pound_force * foot, "N-m");
    } catch (UnitException e) {
      throw new Error(e.getMessage());
    }
    ...
    try {
      ...
      addCoreUnit("slug-ft^2", slug * foot2, "kg-m^2");
    } catch (UnitException e) {
      throw new Error(e.getMessage());
    }
    ...
    try {
      addUnit("Pa", Pa);
      addCoreUnit("psi", psi, "Pa");
      addCoreUnit("psf", psf, "Pa");
      addCoreUnit("atm", atm, "Pa");
      addCoreUnit("mmHg", mmHg, "Pa");
      addCoreUnit("inHg", inHg, "Pa");
      addCoreUnit("bar", bar, "Pa");
      addCoreUnit("mbar", mbar, "Pa");
      addSimilarUnit("pascal", "Pa");

      addUnit("kg/m^3", kilogram / meter3);
      addCoreUnit("slug/ft^3", slug / foot3, "kg/m^3");

      addUnit("kg/m^3-s", kilogram / (meter3 * second));
      addCoreUnit("slug/ft^3-s", slug / foot3, "kg/m^3-s");
    } catch (UnitException e) {
      throw new Error(e.getMessage());
    }
   ...
   * converting each individually (and incuring the string->value lookup
   * penality each time), perform the string lookup once (using this method),
   * then convert each value with the factor.
   * <p>
   * 
   ...
   * <tt> Units.addUnit("knot/s", Units.knot / Units.s, "m/s^2"); </tt>
   * <p>
   * This example creates a new acceleration unit: knots per second. A caveat
   * of this approach is that the base unit must be multiplicative combination
   * of base units (m, s, kg, etc.).
   * <p>
   * The base unit should never be "degreeC" or "degreeF".
   * 
   * @param name
   *            the string name for unit being created.
   * @param factor
  ...
  public static void addUnit(String name, double factor, String base)
      throws UnitException {
    if (base.equals("unspecified")) {
      throw new UnitException("Cannot base any unit on the the "
          + "\"unspecified\" unit.");
    }
  ...
  /**
   * Adds a fundamental unit, where the supplied factor is the factor between
   * the given unit and internal units. This method should only be used when
   * adding a unit that has been specified as a constant factor, but still
   * needs to be added to the database of units.
   * 
   */
  private static void addCoreUnit(String name, double factor, String base)
      throws UnitException {
    if (base.equals("unspecified")) {
      throw new UnitException("Cannot base any unit on the the "
          + "\"unspecified\" unit.");
    ...
    addNewUnitPair(name, new UnitPair(factor, getUnitPair(base).compatible));
  }
   ...
   * so, then do nothing, if not then use the default units.
   * <p>
   * 
   ...
   * @return string with converted value with 2 decimal places, labeled with hte provided unit
   * @throws UnitException
   */
  public static String str(String unit, double value) throws UnitException {
    return str(unit,value,2);
  }
   ...
   * @param value value in internal units
   * @param unit unit to convert to
   * @return string with converted value with no formatting restrictions, labeled with hte provided unit
   * @throws UnitException
   */
   ...
   */
  public static void main(String[] argv) throws UnitException, IOException {
    PrintWriter pw = new PrintWriter(new FileOutputStream("units.html"));
    ...
    pw.println();
    pw.println("<body>");
    pw.println("<H1>List of Units</H1>");
    pw.println("Note: this is the list of units when the ");
    pw.println("Units class was created, the user may have added");
    pw.println("other units in the interm.<p>");
    pw.println("<table border>");
    pw.println("<tr><th>Unit</th>");
    pw.println("    <th>Factor</th>");
    pw.println("    <th>Compatible Units</th></tr>");
