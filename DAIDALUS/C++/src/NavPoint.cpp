...
const NavPoint& NavPoint::INVALID() {
	static NavPoint* v = new NavPoint(Position::INVALID(), std::numeric_limits<double>::quiet_NaN());
	return *v;
}
...
NavPoint NavPoint::makeFull(const Position& p, double t, WayType ty, const std::string& label,
     ...
     return NavPoint(p, t, ty, label, tcp_trk, tcp_gs, tcp_vs, accel_trk,  accel_gs,  accel_vs,  velocityIn, sourcePosition,  sourceTime);
 }
...
bool NavPoint::mergeable(const NavPoint& point) const {
	...
	return r3 && this->t == point.t && this->p == point.p && (this->velocityIn_v.isInvalid() || point.velocityIn_v.isInvalid() ||  this->velocityIn_v == point.velocityIn_v);
}
...
//					accel_trk, accel_gs, accel_vs, velocityIn, sourcePosition, sourceTime);
	...
	return NavPoint(this->p, this->t, my_ty, my_label, my_tcp_trk, my_tcp_gs, my_tcp_vs,
			my_accel_trk, my_accel_gs, my_accel_vs, my_velocityIn, my_sourcePosition, my_sourceTime);
}
...
std::string NavPoint::metaDataLabel(int precision) const {
	...
	if (!velocityIn_v.isInvalid()) {
		std::string v = velocityIn_v.toStringNP(precision);
		replace(v, ',', '_');
		replace(v, ' ', '_');
		tlabel = tlabel + "VEL:"+v+":";
	}
...
//  const std::string& NavPoint::strTCP() const {
//    return toStringTCP(tcp);
//  }


//  bool NavPoint::isFixedTime() const {
//    return mut.time == Fixed;
//  }
//
//  bool NavPoint::isFixedPosition() const {
//    return mut.pos == Fixed;
//  }
//
//  bool NavPoint::isFixedAlt() const {
//    return mut.alt == Fixed;
//  }
//
//  bool NavPoint::isFixedGs() const {
//    return mut.gs == Fixed;
//  }
//
//
//  bool NavPoint::isFixed() const {
//    return isFixedTime() || isFixedPosition() || isFixedAlt() || isFixedGs();
//  }

//  const std::string& NavPoint::toStringMutabilityImpl(NavPoint::Mutability m) const {
////    if (m == NavPoint::Fixed) return NavPoint::FIXED_STR;
////    if (m == NavPoint::Mutable) return NavPoint::MUTABLE_STR;
//    return NavPoint::ELSE_STR;
//  }
//
//  const std::string& NavPoint::strTime() const {
//    return toStringMutabilityImpl(mut.time);
//  }
//
//  const std::string& NavPoint::strPosition() const {
//    return toStringMutabilityImpl(mut.pos);
//  }
//
//  const std::string& NavPoint::strAltitude() const {
//    return toStringMutabilityImpl(mut.alt);
//  }


//	bool NavPoint::isMinorVelChange() const {
//		return minorV_i != 0;
//	}
//
//	bool NavPoint::isMinorTrkChange() const {
//		return (minorV_i & 1) != 0;
//	}
//
//	bool NavPoint::isMinorGsChange() const {
//		return (minorV_i & 2) != 0;
//	}
//
//	bool NavPoint::isMinorVsChange() const {
//		return (minorV_i & 4) != 0;
//	}
//
//	const NavPoint NavPoint::makeMinorTrkChange() const {
//		return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp,
//				this->sourcePosition, this->sourceTime, this->accel_d, this->velocityIn_v, (this->minorV_i|1));
//	}
//
//	const NavPoint NavPoint::makeMinorGsChange() const {
//		return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp,
//				this->sourcePosition, this->sourceTime, this->accel_d, this->velocityIn_v, (this->minorV_i|2));
//	}
//
//	const NavPoint NavPoint::makeMinorVsChange() const {
//		return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp,
//				this->sourcePosition, this->sourceTime, this->accel_d, this->velocityIn_v, (this->minorV_i|4));
//	}
//
//	const NavPoint NavPoint::makeClearMinorVelChange() const {
//		return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp,
//				this->sourcePosition, this->sourceTime, this->accel_d, this->velocityIn_v, (this->minorV_i|(~7)));
//	}

//	const NavPoint NavPoint::makeMarked(unsigned int v) const {
//		return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp,
//				this->sourcePosition, this->sourceTime, this->accel_d, this->velocityIn_v, (this->minorV_i|(v<<3)));
//	}

//	unsigned int NavPoint::getMark() const {
//		return this->minorV_i >> 3;
//	}

double NavPoint::turnRadius() const {
	//fpln("NavPoint::turnRadius "+Fmb(isTurn())+" "+Fm2(accel_d)+" "+velocityIn_v.toString());
	if (isTrkTCP() && accel_trk != 0.0) {
...
//    return NavPoint(this->p, this->t, this->ty, mut, this->label, this->tcp,
//    		this->sourcePosition, this->sourceTime, this->accel_d, this->velocityIn_v, this->minorV_i);
//  }
...
const NavPoint NavPoint::makeSource(const Position& sp, double st) const {
	return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp_trk, this->tcp_gs, this->tcp_vs,
			this->accel_trk, this->accel_gs, this->accel_vs, this->velocityIn_v, sp, st);
}


const NavPoint NavPoint::makeSourcePosition(const Position& sp) const {
	return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp_trk, this->tcp_gs, this->tcp_vs,
			this->accel_trk, this->accel_gs, this->accel_vs, this->velocityIn_v, sp, this->sourceTime_d);
}

const NavPoint NavPoint::makeSourceTime(double st) const {
	return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp_trk, this->tcp_gs, this->tcp_vs,
			this->accel_trk, this->accel_gs, this->accel_vs, this->velocityIn_v, this->sourcePosition_p, st);
}
...
const NavPoint NavPoint::makeTrkAccel(double a) const {
	return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp_trk, this->tcp_gs, this->tcp_vs,
			a, this->accel_gs, this->accel_vs, this->velocityIn_v, this->sourcePosition_p, this->sourceTime_d);
}

const NavPoint NavPoint::makeGsAccel(double a) const {
	return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp_trk, this->tcp_gs, this->tcp_vs,
			this->accel_trk, a, this->accel_vs, this->velocityIn_v, this->sourcePosition_p, this->sourceTime_d);
}

const NavPoint NavPoint::makeVsAccel(double a) const {
	return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp_trk, this->tcp_gs, this->tcp_vs,
			this->accel_trk, this->accel_gs, a, this->velocityIn_v, this->sourcePosition_p, this->sourceTime_d);
}

const NavPoint NavPoint::makeVelocityIn(const Velocity& vi) const {
	return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp_trk, this->tcp_gs, this->tcp_vs,
			this->accel_trk, this->accel_gs, this->accel_vs, vi, this->sourcePosition_p, this->sourceTime_d);
}
...
const NavPoint NavPoint::makeAdded() const {
	...
	return NavPoint(this->p, this->t, this->ty,  this->label_s, this->tcp_trk, this->tcp_gs, this->tcp_vs,
			this->accel_trk, this->accel_gs, this->accel_vs, this->velocityIn_v, Position::INVALID(), -1.0);
}
...
const NavPoint NavPoint::makeBOT(const Position& p, double t,
	...
	return NavPoint(p, t, this->ty,  this->label_s, BOT, this->tcp_gs, this->tcp_vs,
			omega, this->accel_gs, this->accel_vs, v_velocityIn, this->sourcePosition_p, this->sourceTime_d);
}
...
const NavPoint NavPoint::makeEOT(const Position& p, double t, const Velocity& v_velocityIn) const {
	return NavPoint(p, t, this->ty,  this->label_s, EOT, this->tcp_gs, this->tcp_vs,
			this->accel_trk, this->accel_gs, this->accel_vs, v_velocityIn, this->sourcePosition_p, this->sourceTime_d);
}
...
const NavPoint NavPoint::makeBGS(const Position& p, double t,  double d_gsAccel, const Velocity& v_velocityIn) const {
	return NavPoint(p, t, this->ty,  this->label_s, this->tcp_trk, BGS,  this->tcp_vs,
			this->accel_trk, d_gsAccel, this->accel_vs, v_velocityIn, this->sourcePosition_p, this->sourceTime_d);
}
...
const NavPoint NavPoint::makeEGSBGS(const Position& p , double t, double a, const Velocity& v_velocityIn) const {
	return NavPoint(p, t, this->ty,  this->label_s, this->tcp_trk, EGSBGS, this->tcp_vs,
			this->accel_trk, a, this->accel_vs, v_velocityIn, this->sourcePosition_p, this->sourceTime_d);
}
...
const NavPoint NavPoint::makeBVS(const Position& p, double t, double d_vsAccel, const Velocity& v_velocityIn) const {
	return NavPoint(p, t, this->ty,  this->label_s,  this->tcp_trk, this->tcp_gs, BVS,
			this->accel_trk, this->accel_gs, d_vsAccel, v_velocityIn, this->sourcePosition_p, this->sourceTime_d);
}
...
const NavPoint NavPoint::makeEVSBVS(const Position& p , double t, double a, const Velocity& v_velocityIn) const {
	return NavPoint(p, t, this->ty,  this->label_s, this->tcp_trk, this->tcp_gs, EVSBVS,
			this->accel_trk,  this->accel_gs, a, v_velocityIn, this->sourcePosition_p, this->sourceTime_d);
}
...
const NavPoint NavPoint::makeMovedFrom(const NavPoint& o) const {
	return NavPoint(this->p, this->t, o.ty, o.label_s, o.tcp_trk, o.tcp_gs, o.tcp_vs,
			o.accel_trk, o.accel_gs, o.accel_vs, o.velocityIn_v, o.sourcePosition_p, o.sourceTime_d);
...
Velocity NavPoint::velocityIn() const {
	//if (isTCP()) {
	return velocityIn_v;
	//}
...
std::string NavPoint::toString() const {
	...
	//    if (p.isLatLon())
	//      temp <<  Fm4(Units::to(_deg,p.lat())) << ", " << Fm4(Units::to(_deg,p.lon())) << ", " << Fm4(Units::to(_ft,p.alt())) << ", " << Fm4(t);
	//    else
	//      temp << Fm4(Units::to(_NM,p.x())) << ", " << Fm4(Units::to(_NM,p.y())) << ", " << Fm4(Units::to(_ft,p.z())) << ", " << Fm4(t);
	//    return temp.str();
}

std::string NavPoint::toString(int precision) const {
	return p.toStringNP(precision) + ", " + Fm4(t);
}
...
std::vector<std::string> NavPoint::toStringList(int precision, bool tcp) const {
	...
	if (tcp) {
		...
		ret.insert(ret.end(),vec.begin(),vec.end()); // source position (15-17)
		ret.push_back(FmPrecision(sourceTime_d,precision)); // source time (18)
		if (label_s.size() > 0) {
			ret.push_back(label_s); // label (string) (19)
		} else {
			ret.push_back("-");
		}
	} else {
...
NavPoint NavPoint::parseLL(const std::string& s) {
	...
	if (fields.size() == MIN_OUTPUT_COLUMNS+1) {
		...
		LatLonAlt slla = LatLonAlt::parse(fields[14]+" "+fields[15]+" "+fields[16]);
		Position sp = Position(slla);
		double st = Util::parse_double(fields[17]);
		std::string lab = fields[18];
		return NavPoint(pos, time, wt, lab,	trkty, gsty, vsty, trkacc, gsacc, vsacc, vv, sp, st);
	} else {
...
NavPoint NavPoint::parseXYZ(const std::string& s) {
	...
	if (fields.size() == MIN_OUTPUT_COLUMNS+1) {
		...
		Vs_TCPType vsty = Vs_TCPTypeValueOf(fields[12]);
		double vsacc = Units::from("m/s^2", Util::parse_double(fields[13]));
		Vect3 sv = Vect3::parse(fields[14]+" "+fields[15]+" "+fields[16]);
