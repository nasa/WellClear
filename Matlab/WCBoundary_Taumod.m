% +----------------------------------------------------------------------+
% |   Filename           : WCBoundary_Taumod.m                           |
% |   Description        : Well-clear model based on TCAS II mod tau     |
% |   Created by         : C. Munoz, J. Upchurch, A. Narkawicz           |
% |   Created on         : 12 December 2013                              |
% |   Date last modified : 2 January 2014                                |
% +----------------------------------------------------------------------+

function [ Rxy, Dcpa, Taumod, Rz, Tcoa, WCVxy, WCVz, WCV ] = ...
    WCBoundary_Taumod(DTHR,TAUMOD,ZTHR,TCOA, ...
...
tcpa = -dot(s,v)/dot(v,v); % time to closest point of approach
s_tcpa = s+tcpa*v; % relative position at closest point of approach
Dcpa = sqrt(dot(s_tcpa,s_tcpa)); % distance at TCPA
Rxy = sqrt(dot(s,s)); % horizontal range
cxy = dot(s,v); % Cxy < 0 iff aircraft are converging in horizontal plane
if cxy < 0
    Taumod = (DTHR^2-dot(s,s))/dot(s,v); % modified tau
else 
    Taumod = -1;
end

%============= HORIZONTAL WELL CLEAR VIOLATION DEFINITION =================
WCVxy = Rxy <= DTHR || (Dcpa <= DTHR && 0 <= Taumod && Taumod <= TAUMOD);
%==========================================================================
